...
vec3 calc_spot_light(Spot_light light, vec3 norm, vec3 view_direction){
	...
	if(blinn){
		...
		spec = pow(max(dot(norm, halfway_direction), 0.0), material.shininess);
	}else{
		//phong
		vec3 reflection_direction = reflect(-light_direction, norm);
		spec = pow(max(dot(view_direction, reflection_direction), 0.0), material.shininess);
	}
	
	vec3 ambient = vec3(texture(material.diffuse, fs_in.texture_coordinates)) * light.ambient;
	vec3 diffuse = diff * vec3(texture(material.diffuse, fs_in.texture_coordinates)) * light.diffuse;
	vec3 specular = vec3(texture(material.specular, fs_in.texture_coordinates)) * spec * light.specular;
...
vec3 calc_directional_light(Directional_light light, vec3 norm, vec3 view_direction){
	...
	vec3 ambient  = vec3(texture(material.diffuse, fs_in.texture_coordinates)) * light.ambient;
	vec3 diffuse  = vec3(texture(material.diffuse, fs_in.texture_coordinates)) * diff * light.diffuse;
	vec3 specular = vec3(texture(material.specular, fs_in.texture_coordinates)) * spec * light.specular;
